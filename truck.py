...
class Matcher():
	...
	def compareTwoFrame(self,mapFrame1, mapFrame2):
		...
		self.deltaVect[0] = int(generalVect[0] / countOfChangedPoints)
		self.deltaVect[1] = int(generalVect[1] / countOfChangedPoints)

		self.delta = int(sumSpeed/countOfChangedPoints) * sign(directionSum)
		print(self.deltaVect, math.sqrt(self.deltaVect[0]*self.deltaVect[0] + self.deltaVect[1]*self.deltaVect[1] ) , self.delta)
	...
	def calcMapShift(self):
		...
		if self.sideWhereMoveingFrom == 1:
			self.mapShift = Map(self.map2.map[ 0:self.map2.map.shape[0], self.map2.map.shape[1] + self.deltaVect[0]:self.map2.map.shape[1] ])
		elif self.sideWhereMoveingFrom	 == 0:
			self.mapShift = Map(self.map1.map[0:self.map1.map.shape[0], 0:self.deltaVect[0]])
	...
	for pair in framePairs:
		...
		if initFrames:
			...
			if matcher.sideWhereMoveingFrom == 1:
				...
				piece.start(matcher.contourCoor[1], abs(matcher.deltaVect[0]), matcher.map2, matcher.sideWhereMoveingFrom)
		...
		piece = Piece()
		if matcher.sideWhereMoveingFrom == 1:
			piece.start(matcher.map2.map.shape[1] + matcher.deltaVect[0], abs(matcher.deltaVect[0]), matcher.map2, matcher.sideWhereMoveingFrom)
		elif matcher.sideWhereMoveingFrom == 0:
			piece.start(0 + matcher.deltaVect[0], abs(matcher.deltaVect[0]), matcher.map2, matcher.sideWhereMoveingFrom)
			
		truck.update(matcher.map2, matcher.deltaVect[0], matcher.sideWhereMoveingFrom)
